...
namespace Skladista
{
    public partial class Upravljanje_otpremnicom : Form
    {
        ...
        {
            int vrstaDokumenta = 2; // U bazi 2 pretstavlja otpremnicu
            using (var db = new PISkladiste2Entities())
            {
                List<eksterniDokument> otpremnica = (from t in db.eksterniDokument
                                                       where t.vrsta == vrstaDokumenta
                                                       select t).ToList();



                eksterniDokumentBindingSource.DataSource = otpremnica;
            }
        }
